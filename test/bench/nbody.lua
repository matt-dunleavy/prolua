-- floats: the n-body simulation (Computer Language Benchmarks Game)
...
local function offsetMomentum(b, nbody)
  ...
  for i = 1, nbody do
    local bi = b[i]
    local bim = bi.mass
    px = px + (bi.vx * bim); py = py + (bi.vy * bim); pz = pz + (bi.vz * bim)
  end
...
local e0 = energy(bodies, nbody)
for i = 1, N do advance(bodies, nbody, 0.01) end
io.write(string.format("%0.9f %0.9f  %.3fs\n", e0, energy(bodies, nbody), os.clock() - t))
